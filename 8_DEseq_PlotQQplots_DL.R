...
res <- read_tsv("/rs/rs_grp_il9/Il9scRNA/7_outputs_DESeq_ConditionsByCluster_res0.5_IL9_DL/ALL.combined.2024-02-19.tsv")
res <- res %>% separate(cname,c("Location","Cell_type"),sep="_",remove=FALSE)
res <- res %>% filter(!is.na(padj))


## cluster colors 
cell.type.annotation<-read.delim("cell-type.annotation-res0.5.txt")
...
p1 <- res2 %>%
    ...
    guides(colour = guide_legend(override.aes = list(size=1),title="Cell Type")) +
...
ggsave(fname,p1,width=10,height=4)
