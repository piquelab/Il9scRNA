...
outFolder <- paste0("7_outputs_DESeq_ConditionsByCluster_res0.5_IL9_DL/")
system(paste0("mkdir -p ",outFolder))

## Load cells
sc <- readRDS("/rs/rs_grp_il9/Il9scRNA/1_soupx_doubletfinder_chrM/sc.NormByLibrary.cellclassify_newfilter-res0.5.2023-12-27.rds")
md <- sc@meta.data
##md<-md %>% mutate(Pregnancy_ID=str_match(Library,"[0-9]+"))

cSize <- md %>% dplyr::count(Location,seurat_clusters,SampleindexID,Condition)
...
resList <- lapply(1:nrow(kSize),function(ii){
    ...
    bti <- md_i %>% transmute(bti=paste(Condition,SampleindexID,sep="_")) %>% unlist %>% factor
    ...
    ##
    cvt$Group = relevel(factor(cvt$Group),"PBS")
    ##
    ##
    ## Running DEseq2 on the aggregate
    ## ========================================================
...
res %>% filter(padj<0.1,abs(log2FoldChange)>0.0) %>%
    write_tsv(paste0(outFolder,"SIG.combined.2024-02-19.tsv"))
